/* ==============================================================
   VILLA EMILIA — SUSTAV REZERVACIJE
   --------------------------------------------------------------
   • kalendar s odabirom raspona datuma i prikazom zauzetosti
   • zauzetost iz tri izvora: ručni unos (data.js), sinkronizacija
     s Booking.com/Airbnb (availability.json) i vlasnički panel
     (admin.html — sprema se u preglednik vlasnice)
...
(function () {
  ...
  /* ==============================================================
     ZAUZETOST — spaja tri izvora
     1) VS.units[].booked      → ručni unos u data.js
     2) VS.synced[unitId]      → availability.json (Booking.com / Airbnb)
     3) admin blokade          → vlasnički panel (admin.html)
     ============================================================== */
  VS.synced = VS.synced || {};          /* puni se iz availability.json */
  VS.syncedAt = null;

  function adminBlocks(unitId) {
    try {
      var raw = localStorage.getItem("ve-admin-blocks");
      if (!raw) return [];
      var all = JSON.parse(raw);
      return (all && all[unitId]) || [];
    } catch (e) { return []; }
  }

  function allRanges(u) {
    u = u || unit();
    return (u.booked || [])
      .concat(VS.synced[u.id] || [])
      .concat(adminBlocks(u.id));
  }
  VS.allRanges = allRanges;

  function isBooked(date, u) {
    var s = d2s(date);
    return allRanges(u).some(function (r) { return s >= r[0] && s <= r[1]; });
  }

  /* Dohvat sinkroniziranih termina. Datoteku availability.json generira
     serverska skripta iz iCal linkova portala (upute u README.md).
     Ako datoteke nema ili se stranica otvara s diska, tiho se preskače. */
  function loadAvailability(done) {
    var url = (VS.property && VS.property.availabilityUrl) || "";
    if (!url || !window.fetch || location.protocol === "file:") { done(); return; }
    fetch(url, { cache: "no-store" })
      .then(function (r) { return r.ok ? r.json() : null; })
      .then(function (data) {
        if (data && data.units) {
          VS.synced = data.units;
          VS.syncedAt = data.generatedAt || null;
        }
        done();
      })
      .catch(function () { done(); });
  ...
  function renderSyncInfo() {
    ...
    if (!el) return;
    var txt = VS.t("book.cal.sync");
    if (VS.syncedAt) {
      var d = new Date(VS.syncedAt);
      if (!isNaN(d)) {
        txt += " " + VS.t("book.cal.synced") + " " + VS.dateLong(d) + ", " +
  ...
  function checkout(payload) {
    ...
    /* ---------- DEMO NAČIN ----------
       Rezervaciju spremamo u preglednik da se vidi u vlasničkom panelu
       (admin.html → Rezervacije). U pravoj postavi to radi server nakon
       Stripe webhooka. */
    try {
      var log = JSON.parse(localStorage.getItem("ve-bookings") || "[]");
      log.unshift({
        ref: payload.reference, unit: payload.unitId, unitName: payload.unitName,
        ...
        email: payload.guest.email, phone: payload.guest.phone,
        total: payload.price.total, paid: payload.price.payNow,
        payMode: payload.payMode, source: "Direktno", createdAt: new Date().toISOString()
      });
      localStorage.setItem("ve-bookings", JSON.stringify(log.slice(0, 100)));
    } catch (e) {}
  ...
  VS.renderBooking = function () {
    ...
    /* jednom dohvati sinkronizirane termine s portala, pa osvježi kalendar */
    if (!VS.renderBooking._synced) {
      VS.renderBooking._synced = true;
